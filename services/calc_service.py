...
def calc_sales(sheet, creds):
    ...
    for row in sales_data:
        ...
        discount_values.append([round(discount_value, 2)])

        # Calculate VAT value
        vat_percentage = parse_float(row[18]) if len(row) > 18 else 0  # Sales!S
        vat_value = total_amount * (vat_percentage / 100)
    ...
    batch_update(sheet.id, "Sales!R4:R", discount_values, creds)
